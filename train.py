...
from dora import DORA, DORALayer, MagnitudeLayer
...
def replace_linear(model:nn.Module, linear_replacement:nn.Module, 
                   quant_config_4bit:dict|None=None, quant_config_2bit:dict|None=None,
    ...
    """
    # {"q_proj":4, "k_proj":4, "v_proj":4, "o_proj":4, "gate_proj":2, "up_proj":2, "down_proj":2}
    ...
    for name, module in model.named_children():
        ...
        if len(list(module.children())) > 0:
            replace_linear(module, linear_replacement, quant_config_4bit, quant_config_2bit, skip_modules, **kwargs)
...
def fsdp_main(local_rank:int, world_size:int, args:Dict):
    ...
    if args["train_type"] in ["full", "lora", "custom_lora", "custom_dora"]:
        ...
        with init_empty_weights():
            ...
            if args["train_type"] in ["hqq_lora", "hqq_dora", "hqq_llama_pro"]:
                ...
                quant_config_2bit = BaseQuantizeConfig(nbits=2, group_size=64, quant_zero=False,
                                                  quant_scale=False, offload_meta=False, view_as_float=True, axis=1)
                ...
                model.model = replace_linear(model.model, HQQLinear, quant_config_4bit, quant_config_2bit, device=rank,
                                             compute_dtype=compute_dtype, del_orig=True, initialize=False, skip_modules=skip_modules)
        ...
        if rank == 0 or args['verbose']:  print("Loading model", rank)
        if rank == 0 and args['verbose']: print(f"Total model params: {param_count}")
        ...
        for filename in tqdm(files, desc="Loading & Quantizing Model Shards", disable=rank!=0, position=0):
            if args['use_cla']:
                weights = safetensors.torch.load_file(args['cla_model_path'])	
            else:
    ...
    elif args["train_type"] in ["custom_qlora", "custom_lora", "custom_dora", "hqq_lora", "hqq_dora", "bnb_dora"]:
        if args["train_type"] in ["hqq_dora", "bnb_dora", "custom_dora"]:
            if args["train_type"] == "hqq_dora": print("Using HQQDORA", rank)
            if args["train_type"] == "bnb_dora": print("Using BNBDORA", rank)
            if args["train_type"] == "custom_dora": print("Using DORA", rank)
            lora_cls = DORA
        elif args["train_type"] == "custom_lora":
    ...
    my_auto_wrap_policy = get_wrapping_policy(custom_policy=args["train_type"] in ["custom_qlora", "custom_dora", "hqq_lora", "hqq_dora", "bnb_dora"], 
    ...
    if args["save_model"]:
        ...
        if args["train_type"] in ["custom_lora", "custom_dora", "custom_qlora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]:
            cpu_state_dict = {}
            if args["train_type"] in ["bnb_llama_pro", "hqq_llama_pro"]:
...
def main(
    ...
    train_type: Param("", choices=["full", "lora", "qlora", "custom_qlora", "custom_lora", "custom_dora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]) = "qlora", # "full", "lora", "qlora", or "custom_qlora"
    ...
    cla_model_path: str = None, # Prepared model weights for CLA DoRA training
):
