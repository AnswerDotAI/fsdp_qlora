...
DATASET_NAME=$HOME_DIR/data/qwen_large_mix_dataset_v0_dedup_1536
...
cd $HOME_DIR/git/fsdp_qlora && python train.py \
    ...
    --precision $PRECISION \
    --groupsize_2bit $GROUPSIZE_2BIT \
    --block_influence_layers $BI_55_PCT \
    --lr $BASE_LR \
    --lr_div_factor $LR_DIV_FACTOR \
    --disc_lr $DISC_LR \
    --train_layernorms $TRAIN_LAYERNORMS \
    ...
    --log_to wandb \
    --project_name qwen_32b_qdora \
    --name qwen-32b-instruct-dora-4-2bit-block-influence-no-adj-55pct \
    --verbose true \
    --save_model true \
    --save_model_every_n_step $SAVE_STEPS \
    --stop_training_at_step $STOP_STEP \
    --output_dir $SAVE_DIR/qwen-32b-instruct-dora-4-2bit-block-influence-no-adj-55pct 2>&1 | tee $LOG_DIR/qwen-32b-instruct-dora-4-2bit-block-influence-no-adj-55pct.log

# stop azure vm in resource group with name
# az vm stop --resource-group resource-group-us-central --name efficiency-training
